    ...
    def single_run(self):
        ...
        if self.now_modeo == self.cv_modeo[0] or self.now_modeo == self.cv_modeo[1]:
            ...
            if self.blink_detector.response_len() < blink_init_frames:
                ...
                self.center_q1.add_response(
                    cv2.mean(safe_crop(gray_frame, center_x - max(20, radius), center_y - max(20, radius),
        ...
        if old_mode:
            ...
            if not blink_bd and self.blink_detector.enable_detect_flg:
                cv2.threshold(frame_gray_crop, (min_val + thresh_add + self.center_q1.quartile_1) / 2, 255,
                              cv2.THRESH_BINARY_INV, dst=th_frame)
                cv2.morphologyEx(th_frame, cv2.MORPH_OPEN, self.kernel, dst=fic_frame)
                # cv2.morphologyEx(fic_frame, cv2.MORPH_CLOSE, self.kernel, dst=fic_frame)
...
if __name__ == "__main__":
    ...
    logger.info(
        "video info: size:{}x{} fps:{} frames:{} total:{:.3f} sec".format(int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),
                                                                          ...
                                                                          cap.get(cv2.CAP_PROP_FRAME_COUNT) / cap.get(
                                                                              cv2.CAP_PROP_FPS)))
    if save_img:
        all_point_img = np.zeros((int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)), int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), 3),
                                 dtype=np.uint8)
    cap.release()

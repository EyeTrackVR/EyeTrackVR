    ...
    if not self.config.gui_vrc_native:
        ...
        if self.eye_id in [EyeId.LEFT] and not se:  # left eye, send data to left
            ...
            if self.l_eye_blink == 0.0:
                if last_blink > 0.7:  # when binary blink is on, blinks may be too fast for OSC so we repeat them.
                    for i in range(5):
                        self.client.send_message("/avatar/parameters/LeftEyeLidExpandedSqueeze",
                                                 float(self.l_eye_blink))
                if self.config.gui_eye_falloff:
                    if self.r_eye_blink == 0.0:  # if both eyes closed and DEF is enables, blink
                        self.client.send_message("/avatar/parameters/LeftEyeLidExpandedSqueeze",
                                                 float(self.l_eye_blink))
                        self.client.send_message("/avatar/parameters/RightEyeLidExpandedSqueeze",
                                                 float(self.l_eye_blink))
        ...
        if self.main_config.eye_display_id in [EyeId.RIGHT, EyeId.LEFT]:  # we are in single eye mode
            ...
            self.client.send_message("/tracking/eye/LeftRightVec",
                                     [float(eye_x), float(eye_y), 1.0, float(eye_x), float(eye_y),
                                      1.0])  # vrc native ET
        else:
            se = False
...
class VRChatOSC:
    ...
    def __init__(self, cancellation_event: threading.Event, msg_queue: queue.Queue[tuple[bool, int, int]],
        ...
        self.client = udp_client.SimpleUDPClient(self.config.gui_osc_address,
                                                 int(self.config.gui_osc_port))  # use OSC port and address that was set in the config
    ...
    def run(self):
        ...
        while True:
            if self.cancellation_event.is_set():
                ...
                return
            try:
                (self.eye_id, eye_info) = self.msg_queue.get(block=True, timeout=0.1)  # TODO: Too many values to unpack
            except:
                continue
...
class VRChatOSCReceiver:
    def __init__(self, cancellation_event: threading.Event, main_config: EyeTrackConfig, eyes: []):
        ...
        try:
            self.server = osc_server.OSCUDPServer((self.config.gui_osc_address, int(self.config.gui_osc_receiver_port)),
                                                  self.dispatcher)
        except:

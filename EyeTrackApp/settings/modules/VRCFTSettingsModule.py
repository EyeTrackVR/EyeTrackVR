...
from settings.modules.CommonFieldValidators import (
    check_is_ip_address,
...
class VRCFTSettingsModule(BaseSettingsModule):
    def __init__(self, config, widget_id, **kwargs):
        ...
        self.gui_SqueezeThresholdV1_min = (
            f"-VRCFTSETTINGSSQUEEZETHRESHOLDV1MIN{widget_id}"
        )
        self.gui_SqueezeThresholdV1_max = (
            f"-VRCFTSETTINGSSQUEEZETHRESHOLDV1MAX{widget_id}"
        )
        self.gui_SqueezeThresholdV2_min = (
            f"-VRCFTSETTINGSSQUEEZETHRESHOLDV2MIN{widget_id}"
        )
        self.gui_SqueezeThresholdV2_max = (
            f"-VRCFTSETTINGSSQUEEZETHRESHOLDV2MAX{widget_id}"
        )
        self.gui_EyebrowThresholdRising = (
            f"-VRCFTSETTINGSEYEBROWTHRESHOLDRISING{widget_id}"
        )
        self.gui_EyebrowThresholdLowering = (
            f"-VRCFTSETTINGSEYEBROWTHRESHOLDLOWERING{widget_id}"
        )
        self.gui_OutputMultiplier = f"-VRCFTSETTINGSOUTPUTMULTIPLIER{widget_id}"

    def _add_slider_with_controls(
        self, parent, row, label, var, min_v, max_v, step=0.01
    ):
        slider_length = 160
    ...
    def build(self, parent) -> Iterable:
        ...
        ttk.Entry(parent, textvariable=ip_var, width=16).grid(
        ...
        ttk.Entry(parent, textvariable=port_var, width=10).grid(
        ...
        ttk.Entry(parent, textvariable=out_var, width=16).grid(
            row=row, column=1, sticky="w", padx=8, pady=2
        )
        row += 1
        ...
        slider_frame.grid(
            row=row, column=0, columnspan=5, sticky="ew", padx=0, pady=(4, 0)
        )
        row = 0
        ...
        for label, key, default, min_v, max_v in slider_specs:
            ...
            self._add_slider_with_controls(
                slider_frame, row, label, var, min_v, max_v, step=0.01
            )
            row += 1

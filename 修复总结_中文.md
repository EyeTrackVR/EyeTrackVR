...
该修复实现了三层保护机制，并修复了"重新居中眼睛"功能：

### 1. 防止保存无效的校准数据（osc_calibrate_filter.py）

**位置**：`EyeTrackApp/osc_calibrate_filter.py`，第202-220行

**修改内容**：分别处理偏移量（XOFF/YOFF）和椭圆校准（evecs/axes）：
...
if self.calibration_frame_counter == 0:
    ...
    if not (isinstance(evecs, int) and isinstance(axes, int) and evecs == 0 and axes == 0):
        ...
        # 未收集样本 - 仅保存偏移量（用于"重新居中眼睛"）
        # 不覆盖现有的椭圆校准
        print("\033[93m[WARN] 校准已停止但未收集样本。椭圆校准保留，偏移量已更新。\033[0m")
        self.baseconfig.save()  # 仍然保存以持久化偏移量更改
```

**效果**：
- 当用户点击"停止校准"而未先点击"开始校准"时，椭圆校准数据被保留，偏移量仍会更新
- "重新居中眼睛"功能现在正常工作 - 它更新偏移量而不影响椭圆校准
- 正常校准（有样本）会同时保存偏移量和椭圆数据
